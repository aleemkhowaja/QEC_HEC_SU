 ...
 */
	function citationPatent_Crud(event)
	{
		...
		if(flag)
		{
			...
			var citationpatentData = {}
			$('#citationpatent_form input, #citationpatent_form select, #citationpatent_form textarea').each(function (index)
			{
				if($(this).attr('name') != undefined && $(this).attr('id') != undefined)
				{
					citationpatentData[$(this).attr('name')] = $("#"+$(this).attr('id')).val();
				}
			...
			$.ajax({
				url :url,
				 ...
				 data: JSON.stringify(citationpatentData),
