 ...
 */
	function citationThesis_Crud()
	{
		...
		if(flag)
		{
			...
			var citationThesisData = {}
			$('#citationThesis_form input, #citationThesis_form select, #citationThesis_form textarea').each(function (index)
			{
				if($(this).attr('name') != undefined && $(this).attr('id') != undefined)
				{
					citationThesisData[$(this).attr('name')] = $("#"+$(this).attr('id')).val();
				}
			...
			$.ajax({
				url :url,
				 ...
				 data: JSON.stringify(citationThesisData),
